...
namespace edm {
  ...
  void RootFile::dropProcessesAndReorder(StoredProcessBlockHelper& storedProcessBlockHelper,
    ...
    bool isModified = dropProcessesAndReorderStored(
  ...
  bool RootFile::dropProcessesAndReorderStored(std::vector<std::string>& processesWithProcessBlockProducts,
                                               ...
                                               std::vector<unsigned int>& firstFileToStored,
                                               bool firstInputFile) {
    // This function modifies the contents of the StoredProcessBlockHelper
    // after dropOnInput. The first two arguments are references to the
    // data members that are modified.
    ...
    std::vector<std::string>& processes = processesWithProcessBlockProducts;
    ...
    firstFileToStored.assign(nProcessesInFirstFile, StoredProcessBlockHelper::invalidProcessIndex());
    ...
    for (unsigned int iNew = 0; iNew < nProcessesInFirstFile; ++iNew) {
      ...
      if (iOld != StoredProcessBlockHelper::invalidProcessIndex()) {
        offset += nEntries[iOld];
      }
    }

    std::vector<unsigned int> const& oldCacheIndices = processBlockCacheIndices;
    ...
    for (unsigned int k = 0; k < nOuterLoop; ++k) {
      for (unsigned int j = 0; j < nProcessesInFirstFile; ++j) {
        if (firstFileToStored[j] != StoredProcessBlockHelper::invalidProcessIndex()) {
    ...
    processBlockCacheIndices.swap(newCacheIndices);
