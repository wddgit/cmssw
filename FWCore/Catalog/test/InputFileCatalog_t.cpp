...
TEST_CASE("InputFileCatalog with Rucio data catalog", "[FWCore/Catalog]") {
  ...
  // test the ScitagConfig service itself (there is a separate unit test
  // for that elsewhere). It is using a test replacement for the ScitagConfig
  // service NOT the actual ScitagConfig service.
  // Therefore, the expected output PFNs require the InputFileCatalog
  // to properly pass through the SciTagCategory to the function that
  // appends the scitag to the URL. But the rest of the service logic
  // is not present in the test service (e.g. checking for out of range
  // scitags or whether it has the prefix "root:" ...).
  ...
  SECTION("Default behavior") {
    ...
    SECTION("fileNames") {
      SECTION("Catalog 0") {
        ...
        CHECK(names[1] == "file:/foo/bar?scitag.flow=196664");
    ...
    SECTION("fileCatalogItems") {
      ...
      SECTION("Catalog 0") {
        ...
        CHECK(items[1].fileName(0) == "file:/foo/bar?scitag.flow=196664");
  ...
  SECTION("Embedded behavior") {
    ...
    SECTION("fileCatalogItems") {
      ...
      SECTION("Embedded Catalog 0") {
        CHECK(items[0].fileName(0) == "file:/foo/bar?scitag.flow=196700");
  ...
  SECTION("useLFNasPFNifLFNnotFound") {
    ...
    SECTION("fileNames") {
      SECTION("Catalog 0") {
        ...
        REQUIRE(names.size() == 3);
        CHECK(names[0] == "root://cmsxrootd.fnal.gov/store/foo/bar?scitag.flow=196664");
        CHECK(names[1] == "/tmp/foo/bar?scitag.flow=196664");
        CHECK(names[2] == "root://foobar?scitag.flow=196664");
      }
    }
    ...
    SECTION("fileCatalogItems") {
      ...
      SECTION("Catalog 0") {
        CHECK(items[0].fileName(0) == "root://cmsxrootd.fnal.gov/store/foo/bar?scitag.flow=196664");
        CHECK(items[1].fileName(0) == "/tmp/foo/bar?scitag.flow=196664");
        CHECK(items[2].fileName(0) == "root://foobar?scitag.flow=196664");
      }
      SECTION("Catalog 1") {
        CHECK(items[0].fileName(1) == "root://cmsdcadisk.fnal.gov//dcache/uscmsdisk/store/foo/bar?scitag.flow=196664");
        CHECK(items[1].fileName(1) == "/tmp/foo/bar?scitag.flow=196664");
      }
      SECTION("Catalog 2") {
        CHECK(items[0].fileName(2) == "root://host.domain//pnfs/cms/store/foo/bar?scitag.flow=196664");
        CHECK(items[1].fileName(2) == "/tmp/foo/bar?scitag.flow=196664");
      }
    }
  }
}
