...
namespace edm {
  namespace service {
    ...
    public:
      ...
      void appendScitagToURL(SciTagCategory sciTagCategory, std::string& url) const override;

      // The next 4 functions are intended to be used for
      // testing purposes only.
      unsigned int analysisSciTag(SciTagCategory category) const override;
      unsigned int productionSciTag(SciTagCategory category) const override;
      bool enable() const override;
      bool productionCase() const override;

      static void fillDescriptions(ConfigurationDescriptions& descriptions);

    private:
      std::vector<unsigned int> analysisSciTags_;
      std::vector<unsigned int> productionSciTags_;
      bool enable_;
      bool productionCase_;
    };

    ScitagConfig::ScitagConfig(ParameterSet const& pset)
        : enable_(pset.getUntrackedParameter<bool>("enable")),
          productionCase_(pset.getUntrackedParameter<bool>("productionCase")) {
      ParameterSet analysisPSet = pset.getUntrackedParameter<ParameterSet>("analysis");
      analysisSciTags_ = {analysisPSet.getUntrackedParameter<unsigned int>("primarySciTag"),
      ...
      productionSciTags_ = {productionPSet.getUntrackedParameter<unsigned int>("primarySciTag"),
    ...
    void ScitagConfig::appendScitagToURL(SciTagCategory sciTagCategory, std::string& url) const {
      ...
      if (url.starts_with(kXRootPrefix)) {
        ...
        if (index > static_cast<unsigned char>(SciTagCategory::Undefined)) {
          ...
          throw cms::Exception("ScitagConfig") << "Invalid SciTagCategory value in appendScitagToURL.";
        }
        ...
        unsigned int sciTagValue = productionCase_ ? productionSciTags_[index] : analysisSciTags_[index];
        // Validate that the scitag is in the CMS assigned range
        if (sciTagValue < kCMSSciTagRangeStart || sciTagValue > kCMSSciTagRangeEnd) {
          throw cms::Exception("ScitagConfig")
              << "SciTag value " << sciTagValue << " is outside the CMS assigned range of " << kCMSSciTagRangeStart
              << " to " << kCMSSciTagRangeEnd << ".";
        }

        url.append(std::format("{}", sciTagValue));
      }
    }

    unsigned int ScitagConfig::analysisSciTag(SciTagCategory category) const {
      unsigned char index = static_cast<unsigned char>(category);
      if (index >= analysisSciTags_.size()) {
        throw cms::Exception("ScitagConfig") << "Invalid SciTagCategory value passed to analysisSciTag.";
      }
      return analysisSciTags_[index];
    }

    unsigned int ScitagConfig::productionSciTag(SciTagCategory category) const {
      unsigned char index = static_cast<unsigned char>(category);
      if (index >= productionSciTags_.size()) {
        throw cms::Exception("ScitagConfig") << "Invalid SciTagCategory value passed to productionSciTag.";
      }
      return productionSciTags_[index];
    }

    bool ScitagConfig::enable() const { return enable_; }

    bool ScitagConfig::productionCase() const { return productionCase_; }
